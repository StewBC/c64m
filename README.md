...
c64m boots a real C64 ROM set and runs a broad range of software: BASIC programs, single-file PRG binaries, and games and demos loaded from D64 and G64 disk images (read-only by default, optionally writable). It also attaches `.crt` cartridges (type 0 generic 8K/16K, type 5 Ocean, and type 19 Magic Desk multi-bank) and can save and restore full machine snapshots to `.c64state` files. It handles both PAL and NTSC timing. On an Apple M2 Mac Mini, normal mode tracks the real C64's about **1 MHz** Phi2; turbo max free-runs around **12–16 MHz** with full live rendering and cycle-accurate behaviour (roughly **12–16×** real-time), and warp mode (paint off) is faster still for skip-ahead only.
